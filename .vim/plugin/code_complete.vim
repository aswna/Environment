...
if !exists("g:user_defined_snippets")
    let g:user_defined_snippets = "$HOME/.vim/plugin/my_snippets.vim"
endif
...
let g:template['c']['ifn'] = "#ifndef \<c-r>=GetFileName()\<cr>\<cr>#define \<c-r>=GetFileName()\<cr>".
            \repeat("\<cr>",3)."#endif /* \<c-r>=GetFileName()\<cr> */"
let g:template['c']['main'] = "int main(int argc, char \*argv\[\])\<cr>{\<cr>\<cr>return 0;\<cr>}"
...
let g:template['cpp']['ifn'] = "#ifndef \<c-r>=GetFileName()\<cr>\<cr>#define \<c-r>=GetFileName()\<cr>".
            \repeat("\<cr>",4)."#endif // \<c-r>=GetFileName()\<cr>\<cr>\<up>\<up>"
